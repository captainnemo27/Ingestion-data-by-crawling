...
def get_price_from_text_x(text,surface_in_m2):
    '''
        Get price from text
    ...
    '''
    if (text is not None) and (',' in text): #convention naming pep8
        org_text = text
        text = text.lower()
        text = unidecode(text)
        text = remove_char_x(text)
...
def get_surface_from_text_x(text):
    '''
        Get surface from text
    ...
    if (text is not None) and (',' in text): #convention naming pep8
        ...
        if len(sf_collect)> 0:
            sf = parsing_surface_x(sf_collect[0])
        return sf
    return 0

# GET ROAD WIDTH    
def parsing_alley_frontage_x(text):
    '''
...
def get_road_width_x(text):
    ...
    if (text is not None) and (',' in text): #convention naming pep8
        ...
        if len(alleyaccess)> 0:
            alley = parsing_alley_frontage_x(alleyaccess[0][0])

        if len(alley_access)> 0:
            alley = parsing_alley_frontage_x(alley_access[0][0])
        
        if len(frontage)> 0:
            front = parsing_alley_frontage_x(frontage[0][0])

        return alley,front
    return 0, 0

# GET WIDTH LENGTH
def parsing_width_length_x(s):
    '''
...
def get_width_length_x(text):
    ...
    if (text is not None) and (',' in text): #convention naming pep8
        ...
        if len(widths) > 0:
            w= parsing_width_length_x(widths[0][0])

        if len(lengths) > 0: 
            l= parsing_width_length_x(lengths[0][0])
        ...
        if areas: #convention if naming pep8
            ...
            w= parsing_width_length_x(a[0])
            l= parsing_width_length_x(a[1])
        ...
        if lengthwidths:
            ...
            w= parsing_width_length_x(lw[1])
            l= parsing_width_length_x(lw[0])
        ...
        if widthlength:
            w= parsing_width_length_x(widthlength[0][0])
            l= parsing_width_length_x(widthlength[0][1])
        ...
        if general:
            ...
            w= parsing_width_length_x(g[0])
            l= parsing_width_length_x(g[1])
        ...
        if len(width) > 0: 
            w= parsing_width_length_x(width[0][0]) 

        if len(length) > 0: 
            l= parsing_width_length_x(length[0][0]) 
