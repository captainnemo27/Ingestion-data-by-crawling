...
parser.add_argument("-pf","--pro_flag", type=int, default=12)
parser.add_argument("-i","--index", type=int, default=0)
parser.add_argument("-m","--monthly", type=bool, default=True)
args = parser.parse_args()

SITENAME = args.site.lower()
clean_date = args.date
...
if (clean_date == ""):
    ...
    year = clean_date[0:4]
    month = str(int(clean_date[4:6]) - 1)
    if (month == '0'):
        month = '12'
        year = str(int(year) - 1)
else:
...
# DECLARES
mysql_clean_table = "CLEAN_" + SITENAME.upper() 
...
def main():
    ...
    clean_db_connection = sqlalchemy.create_engine("mysql://{}:{}@{}/{}?charset=utf8mb4".format(des_mysql_user,des_mysql_password,des_mysql_host,des_mysql_db),connect_args={'connect_timeout': 60000}) 
    logger.info("Connect clean database successfully")

    start = time()

    if (not test_mode) and (not monthly):
    ...
    while True:
        try:
            ...
            if ((SITENAME == "alonhadat") or (SITENAME == "rongbay")):
                # PRO_WIDTH_TEXT, PRO_LENGTH_TEXT
                df['PRO_WIDTH_TEXT'] = df.apply(lambda x: get_width_length_x(x['DETAILED_BRIEF'])[0], axis=1)
                df['PRO_LENGTH_TEXT'] = df.apply(lambda x: get_width_length_x(x['DETAILED_BRIEF'])[1], axis=1)

                # ALLEY_TEXT, FRONTAGE_TEXT
                df['ALLEY_TEXT'] = df.apply(lambda x: get_road_width_x(x['DETAILED_BRIEF'])[0], axis=1)
                df['FRONTAGE_TEXT'] = df.apply(lambda x: get_road_width_x(x['DETAILED_BRIEF'])[1], axis=1)

                # SURFACE_TEXT, PRICE_TEXT
                df['SURFACE_TEXT'] = df.apply(lambda x: get_surface_from_text_x(x['DETAILED_BRIEF']), axis=1)
                df['PRICE_TEXT'] = df.apply(lambda x: get_price_from_text_x(x['DETAILED_BRIEF'],x['SURFACE_TEXT']), axis=1)
            else:
                # PRO_WIDTH_TEXT, PRO_LENGTH_TEXT
