...
/class="re__section re__pr-description js__section"/{
...
/<span class="title">Mức giá/{
        getline;
    if (val["PRICE_ORIGINAL"] == "")
        val["PRICE_ORIGINAL"] = cleanSQL($0)
}
/class="re__contact-name js_contact-name"/{
    if (val["DEALER_NAME"] == ""){
        split($0,arr,"title=\"")
        split(arr[2],arr,"\"")
...
/price: '/{
    if (val["PRICE_ORIGINAL"] == ""){
        split(cleanSQL($0),arr,"price: '")
        split(arr[2],arr," ")
        val["PRICE_ORIGINAL"] = arr[1]
    }
}
/priceSort: '/{
    if (val["PRICE"] == ""){
        split(cleanSQL($0),arr,"priceSort: '")
        split(arr[2],arr,"'")
        val["PRICE"] = arr[1]
    }
}
