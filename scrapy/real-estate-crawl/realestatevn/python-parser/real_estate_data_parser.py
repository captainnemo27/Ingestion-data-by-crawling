...
with open(config_file) as yml_file:
    try:
        ...
        if len(ads['ID_CLIENT'])>0:
            ...
            option = "UPDATE IGNORE " + str(table) + ' set PRO_FLAG=0, CREATED_DATE=' + '"' + str(created_day) + '", '
            option_2 = '  WHERE site="'+ str(site) + '" and ID_CLIENT="' + str(ads['ID_CLIENT']) + '";' 
            row = option + sql_query(ads) + option_2
            print(row)
    except:
        pass
