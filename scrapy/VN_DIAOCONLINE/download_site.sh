...
readonly DOMAIN="https://diaoconline.vn"
...
readonly TABLE_NAME="DIAOCONLINE"
...
function download_each_listmode_multi {
    ...
    while [[ ${page} -le ${nb_pages} ]]; do
        ...
        local page_html_file="${LIST_FOLDER}/page-${TOTAL_PAGE}.html"
...
function create_need_download_tab {
    ...
    for p in ${previous_dates[@]}; do
        local status_ok_file="${work_dir}/${p}/DELTA/status_ok"

        if [[ "${most_recent_date}" -lt "${p}" ]] && [[ "${p}" -ne "${folder_name}" ]]; then
            if grep 'OK' "${status_ok_file}"; then
                most_recent_date=${p}
            else
                echo "${previous_dates} no status_ok file"
            fi
        fi
    done
...
function main {
    ...
    if [[ "${mode_download}" -eq 1 ]] && [[ ${get_all_ind} -eq 1 ]]; then
        ...
        # Test mode
            ...
            local base_url="${DOMAIN}/sieu-thi"
            local nb_page=2
